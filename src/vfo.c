...
float calibOffset=0.0; // offset the DttSp RxOsc by this amount to compensate for xtal issues.
...
#else
        sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 20'>% 7lld.%03lld.%03lld </span>",aTransmitting?"#FF0000":"#00FF00",frequencyA/1000000LL,(frequencyA%1000000LL)/1000LL,frequencyA%1000LL);
#endif
...
void vfoSaveState() {
	...
	sprintf(string,"%8.8f",calibOffset);
    setProperty("calibOffset",string);
...
void vfoRestoreState() {
	...
	value=getProperty("calibOffset");
    if(value) calibOffset=atof(value);
