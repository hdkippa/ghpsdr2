...
{
    if (sig == SIGINT) 
		fprintf(stderr, "%s: Caught signal Ctrl+C\n", __func__);
	else
...
// initialize and open a UDP server
int ozy_open_udpserver(int port)
{
	if (init_udp(port))
	{
		fprintf(stderr, "%s: init_udp failed.\n", __func__);
		return -1;
	}
...
// record the I/Q sample stream
FILE *ozy_open_file(const char *path)
{
	FILE *fp;
...
void ozy_close_file(FILE *fp)
{
	if (fp == NULL)
	{
		fprintf(stderr, "%s: No fp given.\n", __func__);
		return;
	}
...
void quit() {
	// close the recFp
	ozy_close_file(recFp);

	// clear libusb buffers
	ozy_close();
...
void buildMainUI() {
	...
	gtk_widget_set_name(mainWindow, APPNAME);
    ...
    //gtk_widget_modify_fg(label, GTK_STATE_NORMAL, &white);newSpectrumDisplay
...
#else
    gtk_fixed_put((GtkFixed*)mainFixed,preampWindow,5,550);
#endif
...
#else
    gtk_fixed_put((GtkFixed*)mainFixed,volumeWindow,5,600);
#endif
...
#else
    gtk_fixed_put((GtkFixed*)mainFixed,displayWindow,210,60);
#endif
...
#else
    gtk_fixed_put((GtkFixed*)mainFixed,bandscopeWindow,210,485);
#endif
    gtk_widget_show(bandscope_controlWindow);
#ifdef NETBOOK
    gtk_fixed_put((GtkFixed*)mainFixed,bandscope_controlWindow,164,454);
#else
    gtk_fixed_put((GtkFixed*)mainFixed,bandscope_controlWindow,210,585);
#endif
...
#else
    gtk_fixed_put((GtkFixed*)mainFixed,transmitWindow,210,615);
#endif
...
#else
    gtk_widget_set_size_request(GTK_WIDGET(subrxWindow),450,85);
    gtk_fixed_put((GtkFixed*)mainFixed,subrxWindow,635,615);
#endif

#ifdef NETBOOK
    gtk_widget_set_size_request(GTK_WIDGET(mainFixed),1024,576);
#else
    //gtk_widget_set_size_request(GTK_WIDGET(mainFixed),1280,1024);
...
void initColors() {
    ...
    verticalColor.red=65535*132/256;
    verticalColor.green=65535*84/256;
    verticalColor.blue=65535*132/256;

    //horizontalColor.red=65535*39/256;
    //horizontalColor.green=65535*15/256;
    //horizontalColor.blue=65535*41/256;

    horizontalColor.red=65535*132/256;
    horizontalColor.green=65535*84/256;
    horizontalColor.blue=65535*132/256;

    spectrumTextColor.red=65535*207/256;
    spectrumTextColor.green=65535*210/256;
    spectrumTextColor.blue=65535*179/256;

	fprintf(stderr, "%s: spectrumTextColor.red %d\n", __func__, spectrumTextColor.red);
	fprintf(stderr, "%s: spectrumTextColor.green %d\n", __func__, spectrumTextColor.green);
	fprintf(stderr, "%s: spectrumTextColor.blue %d\n", __func__, spectrumTextColor.blue);
    meterYellow.red=65535*193/256;
    meterYellow.green=65535*188/256;
    meterYellow.blue=65535*94/256;
...
static struct option longOptions[] = {
    ...
    {"udp server",required_argument, NULL, 'u'},
    {0,0,0,0}
};

static char* shortOptions="c:s:p:tmi:f:u:";
    ...
    while(1) {
        ...
        switch(c) {
            ...
            case 'i':
                ...
                break;
			case 'f':
				recFilePath=strdup(optarg);
				// what if filename begins with a -? then we don't want to use stdout, sigh figure this out
				if (recFilePath && strncmp("-", recFilePath, 1))
					// != to "-", use recFilePath
					ozy_open_file(recFilePath);
				// else we'll be using stdout
				break;
			case 'u':
				ozy_open_udpserver(atoi(optarg));
				break;
            case '?':
...
int main(int argc,char* argv[]) {
    ...
    sigact.sa_handler = main_signal_handler;
    ...
    NewKeyer(800.0f,TRUE,0.0f,3.0f,25.0f,48000.0f);
    SetKeyerPerf(FALSE);
    StartKeyer();
