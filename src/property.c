...
char* getProperty(char* name) {
    ...
    PROPERTY* property=properties;
    while(property) {
		//fprintf(stderr, "%s: wanted %s name %s value %s\n", __func__, name, property->name, property->value); 
	...
	//fprintf(stderr, "%s: wanted %s return name %s value %s\n", __func__, name, property->name, value); 
    return value;
}
