    ...
    if(widget==buttonBand14) {
        ...
        if(displayHF) {
            ...
            if(currentBandButton==widget) {
				...
				// scale the highlight color by the bandstack index
				float scale = (float)(bandstack[band].current_entry+1)/(float)bandstack[band].entries;
				color.red = buttonSelected.red*scale;
				color.green = buttonSelected.green*scale;
				color.blue = buttonSelected.blue*scale;
