...
#else
    sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 20'>% 7lld.%03lld.%03lld </span>",subrx?"#00FF00":"#C0C0C0",subrxFrequency/1000000LL,(subrxFrequency%1000000LL)/1000LL,subrxFrequency%1000LL);
#endif
...
void updateSubrxDisplay() {
    ...
    if(subrxFrequencyDisplay->window) {
        ...
        sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 12'>% 7lld.%03lld.%03lld </span>",subrx?"#00FF00":"#C0C0C0",subrxFrequency/1000000LL,(subrxFrequency%1000000LL)/1000LL,subrxFrequency%1000LL);
#else
        sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 8'>% 7lld.%03lld.%03lld </span>",subrx?"#00FF00":"#C0C0C0",subrxFrequency/1000000LL,(subrxFrequency%1000000LL)/1000LL,subrxFrequency%1000LL);
#endif
        pango_layout_set_markup(layout,temp,-1);
        gdk_draw_layout(GDK_DRAWABLE(subrxPixmap),gc,0,0,layout);
...
GtkWidget* buildSubRxUI() {
...
#else
    subrxTable=gtk_table_new(1,4,TRUE);
#endif

    // subrx settings

    subrxFrequencyDisplay=gtk_drawing_area_new();
    //gtk_widget_set_size_request(GTK_WIDGET(subrxFrequencyDisplay),250,20);
...
#else
    gtk_table_attach_defaults(GTK_TABLE(subrxTable),subrxFrequencyDisplay,1,2,0,1);
#endif
    ...
    gtk_widget_set_size_request(GTK_WIDGET(subrxEnabled),BUTTON_WIDTH/2,BUTTON_HEIGHT);
...
#else
    gtk_table_attach_defaults(GTK_TABLE(subrxTable),subrxGainFrame,2,3,0,1);
#endif
...
#else
    gtk_table_attach_defaults(GTK_TABLE(subrxTable),subrxPanFrame,3,4,0,1);
#endif
