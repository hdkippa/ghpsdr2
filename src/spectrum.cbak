...
gboolean spectrum_button_press_event(GtkWidget* widget,GdkEventButton* event) {
    ...
    switch(event->button) {
        ...
        case 3:
			...
			// eliminate set frequency to a 1 in the Hz value, becuase of bipolar math and zero counter
			increment = increment%frequencyIncrement ? increment-increment%frequencyIncrement : increment;
...
gboolean spectrum_button_release_event(GtkWidget* widget,GdkEventButton* event) {
    ...
    switch(event->button) {
        ...
        case 3:
			...
			// or, move filter edge, pop it to here
            break;
    }
    return TRUE;
}
...
void drawSpectrum(int height) {
    ...
    if(spectrum->window) {
		...
		/*
		fprintf(stderr, "val %d y %d sMAX %d sMIN %d sSTEP %d v %d n %d p %d i %d \n",
		val, y, spectrumMAX, spectrumMIN, spectrumSTEP, v, n, p, i);
		*/
            gdk_gc_set_rgb_fg_color(gc,&horizontalColor);
            ...
            pango_layout_set_markup (layout, label, -1);
            gdk_draw_layout (GDK_DRAWABLE (spectrumPixmap), gc, 0, y-7, layout);
            gdk_draw_layout (GDK_DRAWABLE (spectrumPixmap), gc, spectrumWIDTH-20, y-7, layout);
        }
...
void drawWaterfall(int y,int height) {
    ...
    if(spectrum->window) {
		...
		int rowBytes=nChannels*spectrumWIDTH;
        for( i = height-2; i > 0; i-- ) {
            pix = waterfallPixels + rowStride * i + nChannels;
            //for( j = 0; j < spectrumWIDTH; j+=2 ) {
				memcpy(pix, pix-rowStride, rowBytes);
				/* ohh, dudes, this is not the way to do this
                pix[0] = pix[ -rowStride];
                pix[1] = pix[1-rowStride];
                pix[2] = pix[2-rowStride];

                pix[3] = pix[3-rowStride];
                pix[4] = pix[4-rowStride];
                pix[5] = pix[5-rowStride];
