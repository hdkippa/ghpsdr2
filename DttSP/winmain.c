...
tryagain:
		...
		ls = ringb_float_read_space (top[i].jack.ring.o.l);
		rs = ringb_float_read_space (top[i].jack.ring.o.r);

		if ((ls >= nframes) && (rs >= nframes))
		{
		...
		else
		{
			...
			fprintf(stdout,"Audio_Callback-1: rb out pathology nframes %d read lspace %d rspace %d retry %d\n", nframes, (int)rs, (int)ls, retry), fflush(stdout);
//			reset_system_audio(nframes);
//			memset (output_l, 0, nframes * sizeof (float));
//			memset (output_r, 0, nframes * sizeof (float));
		}
...
tryagain2:
		...
		ls = ringb_float_write_space (top[i].jack.ring.i.l);
		rs = ringb_float_write_space (top[i].jack.ring.i.r);
		...
		{	// rb pathology
			...
			fprintf(stdout,"Audio_Callback-2: rb out pathology nframes %d read lspace %d rspace %d retry %d\n", nframes, (int)rs, (int)ls, retry), fflush(stdout);
