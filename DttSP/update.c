...
SetRXAGC (unsigned int thread, unsigned subrx, AGCMODE setit)
{
	...
	switch (setit)
	{
		case agcOFF:
			...
			rx[thread][subrx].dttspagc.gen->gain.fix = 10000.0;
			break;
		case agcLONG:
			rx[thread][subrx].dttspagc.gen->mode = agcLONG;
			rx[thread][subrx].dttspagc.flag = TRUE;
			rx[thread][subrx].dttspagc.gen->hangtime = (REAL) 0.75;
...
CalculateRXMeter (unsigned int thread, unsigned int subrx, METERTYPE mt)
{
	...
	switch (mt)
	{
		case SIGNAL_STRENGTH:
			...
			/*
			fprintf(stderr, "%s: norm %f val %f avg %f \n", __func__, 
			rx[thread][subrx].norm, 
			uni[thread].meter.rx.val[subrx][SIGNAL_STRENGTH],
			uni[thread].meter.rx.val[subrx][AVG_SIGNAL_STRENGTH]);
			*/
